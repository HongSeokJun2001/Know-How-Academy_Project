					...
					<c:choose>
						...
						<c:otherwise>	
							<c:forEach var="n" items="${ requestScope.list }">
								<tr>
									<td>${ n.rowNum }</td>
									<td><a href="/know-how/admin/notice/detail/${ n.postNo }" class="link">${ n.title }</a></td>
									<td>${ n.postWriter }</td>
									<td>${ n.createdAt }</td>
									<td>
										<c:choose>
											<c:when test="${ n.status eq 'Y' }">
												<span class="status show" onclick="visible('${ n.status }', ${ n.postNo })">노출</span>
											</c:when>
											<c:otherwise>
												<span class="status hide" onclick="visible('${ n.status }', ${ n.postNo })">숨김</span>
											</c:otherwise>
										</c:choose>
									</td>
									<td>
